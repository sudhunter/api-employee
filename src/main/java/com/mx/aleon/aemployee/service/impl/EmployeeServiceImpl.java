...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeVO findById(Long id) {
		...
		return employeeRepository.findById(id).map(mapper::employeeToEmployeeVO)
				.orElseThrow(() -> new ResourceNotFoundException(-1, EMPLOYEE_NOT_FOUND_WITH_ID + id));
	}
	...
	public List<EmployeeVO> findAll() {
		try {
			...
			if (employees.isEmpty()) {
				throw new ResourceNotFoundException(-1, "No employees found.");
			}
	...
	public EmployeeVO save(EmployeeVO employeeVO) {
		try {
			...
			throw new BadRequestException(-1, "Unable to save employee details.");
		}
	}
	...
	public void deleteById(Long id) {
		try {
			...
			if (!employeeRepository.existsById(id)) {
				throw new BadRequestException(-1, EMPLOYEE_NOT_FOUND_WITH_ID + id);
			}
	...
	public EmployeeVO update(EmployeeVO employeeVO) {
	    try {
	        ...
	        Employee existingEmployee = employeeRepository.findById(employee.getId())
	                .orElseThrow(() -> new ResourceNotFoundException(-1, EMPLOYEE_NOT_FOUND_WITH_ID + employee.getId()));
	        ...
	        throw new BadRequestException(-1, "Unable to update employee details.");
	    }
	}
}
